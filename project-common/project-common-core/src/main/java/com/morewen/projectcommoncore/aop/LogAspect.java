package com.morewen.projectcommoncore.aop;
...
public class LogAspect
{
    ...
    protected void handleLog(final JoinPoint joinPoint, Log controllerLog, final Exception e, Object jsonResult)
    {
        try
        {
            Long runTime = System.currentTimeMillis() - TIME_THREADLOCAL.get();
            // 打印请求参数
            String model = controllerLog.title();
            String className = joinPoint.getTarget().getClass().getName();
            String methodName = joinPoint.getSignature().getName();
            Object[] args = joinPoint.getArgs();
            String params = argsArrayToString(args, controllerLog.excludeParamNames());
            if (StringUtils.isEmpty(params)) {
                params = "null";
            }
            log.info("[{}],请求类:{},请求方法:{},请求参数:{},执行时间{}ms,返回结果{}",model, className, methodName, params, runTime, jsonResult);
            if (!Objects.isNull(e)) {
